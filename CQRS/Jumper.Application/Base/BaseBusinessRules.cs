...
public class BaseBusinessRules
{
    ...
    public virtual void ThrowExceptionEntityIsConstant<T>(T entity)
        where T : IConstantEntity
    {
        if(entity.IsConstant)
        {
            throw new BusinessException("Değiştirmeye çalıştığınız veri sabit olduğu için işlem yapamazsınız.");
        }
    }
